...
//Don't change anything beneath this line now
...
function addTrackerResponse(token, result, title, sentToRebble = true, source = "IFTTT") {
    resultTracker[token] = (resultTracker[token] == null) ? [] : resultTracker[token]

    resultTracker[token].push({
        number: parseInt(resultTracker[token].length),
        result: result,
        title: title,
        sentToRWS: sentToRebble,
        source: source,
        time: new Date()
    });
}
